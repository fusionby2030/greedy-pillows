...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims, input_dims, hidden_layer_sizes=[100], condition_dims=None):
        super(VariationalEncoder, self).__init__()

        self.linear1 = nn.Linear(input_dims, hidden_layer_sizes[0])
        self.linear21 = nn.Linear(hidden_layer_sizes[0], latent_dims)
        self.linear22 = nn.Linear(hidden_layer_sizes[0], latent_dims)
    ...
    def forward(self, x):
        ...
        self.kl = (sigma**2 + mu**2- torch.log(sigma) - 1/2).sum() # Calculate KL div
        return z
...
class VariationalAutoencoder(nn.Module):
    def __init__(self, latent_dims, input_dims):
        ...
        self.decoder = Decoder(latent_dims, output_dims=input_dims)
...
def train(autoencoder, train_loader, test_loader=None, optimizer=None, epochs=100, criterion=None, variational=False):
    if optimizer == None:
    ...
    with tqdm(desc="epoch", total=epochs) as pbar_outer:
        for epoch in range(epochs):
            for batch_idx, (x, y) in enumerate(train_loader):
                optimizer.zero_grad()
                x_hat = autoencoder(x)
                loss = criterion(x, x_hat) + autoencoder.encoder.kl if variational else criterion(x, x_hat)
                loss.backward()
                optimizer.step()
            if test_loader:
                val_loss, avg_RMSE = eval_epoch(autoencoder, test_loader, criterion)
                if epoch > epochs-15:
                    last_results.append(avg_RMSE)
            else:
                val_loss = avg_RMSE = 0.0
            nni.report_intermediate_result(-np.log10(avg_RMSE))
            pbar_outer.update(1)
            # tqdm.write(log_template.format(ep=epoch+1, t_loss=loss.item(), v_loss=val_loss, rmse=avg_RMSE))

        nni.report_final_result(-np.log10(np.mean(last_results)))
    return autoencoder
...
def plot_latent(autoencoder, data_loader, title=None):
    ...
    for i, (x, y) in enumerate(data_loader):
        ...
        z = z.detach().numpy()
        plt.scatter(z[:, 0], z[:, 1], c=y, cmap='inferno', s=400)
    # plt.clim(1.0, 12)
    plt.colorbar()
    if title:
        plt.title(title)
...
def main(args):
    ...
    vae = VariationalAutoencoder(latent_dims=args.latent_dims, input_dims=10)
    vae.double()
    ...
    # vae = train(vae, train_loader, test_loader, optimizer=optimizer, epochs=args.epochs)

    # train_loader = torch.utils.data.DataLoader(train_set, batch_size=1)
    # test_loader = torch.utils.data.DataLoader(test_set, batch_size=1)
    # plot_train_and_test(vae, train_loader, test_loader)
    # plt.show()
    ...
    autoencoder = train(autoencoder, train_loader, test_loader, optimizer=optimizer, epochs=args.epochs)

    # plot_latent(autoencoder, train_loader, title="Training set")
...
def search_main(args):
    ...
    for epoch in range(args.epochs):
        for batch_idx, (x, y) in enumerate(train_loader):
            optimizer.zero_grad()
            x_hat = vae(x)
            loss = criterion(x, x_hat) + vae.encoder.kl if variational else criterion(x, x_hat)
            loss.backward()
            optimizer.step()
        if test_loader is not None:
            val_loss, avg_RMSE = eval_epoch(vae, test_loader, criterion)
