...
class Decoder(nn.Module):
    def __init__(self, latent_dims=2, output_dims=10):
        ...
        self.block = nn.Sequential(
        nn.Linear(latent_dims, 100), nn.ReLU(),
...
def train(autoencoder, train_loader, test_loader=None, optimizer=None, epochs=100, criterion=None, variational=False, beta=0.3):
    if optimizer == None:
    ...
    with tqdm(desc="epoch", total=epochs) as pbar_outer:
        for epoch in range(epochs):
            ...
            for batch_idx, (x, y) in enumerate(train_loader):
                ...
                kl_loss = autoencoder.encoder.kl if (variational and epoch > 30) else 0.0
...
def plot_predictions(autoencoder, data_loader):
    fig = plt.figure(figsize=(18, 18))
    for i, (x, y) in enumerate(data_loader):
        z = autoencoder.encoder(x)
        z = z.detach().numpy()
...
def main(args):
    ...
    vae = train(vae, train_loader, test_loader, optimizer=optimizer, epochs=args.epochs, variational=True)
