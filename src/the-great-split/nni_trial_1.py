...
def main(**kwargs):
    ...
    # Train CLF
    CLF, clf_scores = train_classifier(dataset=(low_neped, high_neped, final_exam), autoencoder=AE)
    ...
    nni.report_final_result({'default': clf_scores[2], 'recon_RMSE': RMSE_recon})



if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Great Split Experiment for VAE models')
    parser.add_argument("-bs", "--batch_size", help='batch size for training', type=int, default=437)
    parser.add_argument("-ep", "--epochs", help='num epochs', type=int, default=215)
    ...
    try:
        ...
        if 'h1' in config.keys():
            print('changing hidden layers from search space')
            config['hidden_dims'] = [config['h1'], config['h2'], config['h3'], config['h4']]
        main(**config)
